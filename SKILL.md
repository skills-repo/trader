---
name: trader
description: "趋势交易者技能库（superpower 路由层）：为 AI Agent 提供美股日线趋势交易方法论与加密货币研究决策框架，涵盖交易系统核心（三层过滤、仓位风险公式、入场退出）、持仓与期权决策框架、交易复盘，以及加密货币链上数据解读与 24/7 高波动市场仓位纪律。适用场景：评估个股入场机会、计算仓位与止损、管理持仓与候选池、评估 Bull Call Spread / Long Call 期权策略、单笔交易复盘与月度系统性回顾，以及加密货币研究与现货仓位决策。核心理念：不预测，只跟随；先控制风险，再等待趋势。仅做多美股正股/ETF 与加密货币现货，不使用融资、不做日内交易；加密货币不杠杆、不合约、不链上签名。"
agent_created: true
metadata:
  version: 1.0.0
  category: 交易/投资
